...
- [cs bash 3.2 + BSD compatibility](feedback_bash32_compatibility.md): cs AND tests/ run on stock bash 3.2 + BSD; no bash 4+ (local -A, printf %(...)T, source <()) or GNU-only sed/awk/stat
...
- [cs multi-user safety](project_cs_multi_user_safety.md): classify every new session-dir write path (.cs/local/, union stream, JSON driver); tracked-file dates from git, never the clock
...
- [pipefail SIGPIPE early-exit class](project_pipefail_sigpipe.md): early-exiting consumers (grep -q, sed q) silently kill cs at exit 141 on >64KB input; read files directly, probe with >64KB
...
- [Empty name resolves to sessions root](project_empty_name_root_deletion.md): "" passes the traversal guard to the sessions root (`cs -rm victim ""` deleted all); reject empty names everywhere
...
- [Subagents never dump the environment](project_subagent_no_env_dump.md): `env | grep` wrote live secrets to a subagent transcript; forbid env, printenv, set AND export -p; probe with ${NAME:+set}
...
- [CS_CLAUDE_SESSION_ID is the launch id](project_cs_claude_session_id_is_launch_id.md): stale after the first rotation; read claude_session_id in .cs/local/state; keep the staleness (crash recovery)
...
- [Codex review briefs](project_codex_readonly_no_probes.md): "do not edit files" means no fixtures, so grant a scratch dir; on a dirty tree pin the scope or it asks instead of reviewing
- [Remote Control contract](project_remote_control_contract.md): subscription-only, one account per server, phone sessions lack cs's launch env; `cswap run`, never `cswap switch` under a live server
