...
- [cs memory is Claude Code's, not cs's](project_cs_memory_ownership.md): cs only redirects the path (autoMemoryDirectory + COWORK override); do NOT regenerate MEMORY.md or stamp memory frontmatter from cs — attribute via git log .cs/memory
- [cs multi-user safety is a standing requirement](project_cs_multi_user_safety.md): classify every new session-dir write path (.cs/local/ per-machine, merge=union append streams, semantic driver for JSON); dates in TRACKED files come from git, never the clock
...
- [Plan code needs fixture-reaches-branch check](feedback_plan_code_fixture_check.md): 7/7 defects were plan-born; fix dispatches ARE plans (verify the fix like the feature); keep 'flag, don't silently fix' in every implementer dispatch
...
- [Claude Code bundle is the authoritative contract](reference_claude_code_bundle.md): grep the installed bundle (~/.local/share/claude/versions/<v>) rather than trusting public docs when integrating with a hook/statusline/settings surface
- [No foreground sleep in dispatched agents](project_subagent_no_foreground_sleep.md): the harness blocks it and a reviewer polling with sleep 60 hangs unrecoverably — every long-work dispatch prompt must forbid it explicitly
...
- [Polish before merge](feedback_polish_before_merge.md): at merge gates after a clean final review, Alex picks polish-first (3/3) — recommend it as the first option
...
- [Skill surfaces over cs verbs](user_prefers_skill_surfaces.md): RULE (2026-09-14) — as few cs verbs as possible; every capability reachable from inside Claude Code as a skill/plugin/command; a verb is only hidden plumbing behind one
...
- [tmux target anchor split](project_tmux_target_anchor_split.md): 3.6a takes '=name' on resolvers but rejects it on set/show-option; plain `cs` there is safe ONLY after exact existence is established; test against a real server
- [Empty name resolves to sessions root](project_empty_name_root_deletion.md): "" passes the traversal guard and maps to the sessions root (`cs -rm victim ""` rm -rf'd everything); reject empty names in EVERY position before the first destructive act
...
- [Statusline memo inheritance trap](project_statusline_memo_inheritance.md): a bare shell global memo is seeded by an inherited env var of the same name; use a ready-flag, reset both at main(), 10#-normalize before arithmetic
...
- [Release gate skips CI](project_release_gate_skips_ci.md): Step 0b gates content on CI; Alex's shape: skip the repeat local run, push the release commit, and tag ONLY after its 5 CI jobs are green; local bash is 5.x, floor is 3.2
- [Backend dispatch silent no-op](project_backend_dispatch_silent_noop.md): a bash case with no default arm silently no-ops an unknown value; CI-platform-safe != runtime-platform-safe (an msys arm gated on powershell.exe never fires in CI)
...
- [Validate after the transform](project_validate_after_transform.md): prefer closing the class structurally over a deny-list; where one is unavoidable validate the OUTPUT and test BOTH directions, then measure the rule against real data
...
- [Subagent idle is not dead](project_subagent_idle_not_dead.md): never judge by mtime or an unanswered ping; NAMED agents are tmux-backed (check list-panes) — past a time box re-dispatch the same prompt UNNAMED, or use codex exec with </dev/null
...
- [No real identities in fixtures](feedback_no_real_identities_in_fixtures.md): never put real names, emails, usernames or handles in fixtures, test names, assertion strings, comments or docs — example.com placeholders; keep the shape, drop identities
...
- [Secret-shaped fixtures vs push protection](project_secret_fixtures_push_protection.md): GH013 rejects a literal credential pattern even in a test fixture — assemble tokens from prefixes at runtime, and never take the unblock URL
- [/write-as-me corpus sweeps every project](project_voice_corpus_sweeps_all_projects.md): no exclusion mechanism; an exclusion feature was DECLINED 2026-08-10, do not re-propose; exclude by hand and record it in the profile's Provenance
...
- [Subagents never dump the environment](project_subagent_no_env_dump.md): `env | grep` in a debugging subagent wrote live secrets to its transcript — dispatch prompts forbid env, printenv, set AND export -p; probe one var with ${NAME:+set}
...
- [Rebase without -i](project_rebase_without_interactive.md): `GIT_SEQUENCE_EDITOR=true git rebase -i --autosquash <base>` folds fixups (-i only picks the editor); sed-to-reword + `GIT_EDITOR="cp /tmp/msg"` rewords; verify with git diff HEAD@{1} HEAD
- [Native tasks are per session under cs](project_native_tasks_per_conversation.md): CLAUDE_CODE_TASK_LIST_ID = session name, so the list survives /clear; rotations reconcile, never mirror; needs the opt-in cs exports (CLAUDE_CODE_ENABLE_TODO_TOOLS=1)
...
- [Teammate shares session slots](project_teammate_shares_session_slots.md): a tmux teammate is a full claude in the same session dir with its own Stop and statusline; gate every .cs/local/ slot on the lead for WRITES and for READS (4 defects)
- [Gate: one at a time, --changed for the loop](project_gate_one_at_a_time.md): one background gate at a time, never beside a council; never edit a script mid-run; capture output to a file and grep it, never filter in the pipe
- [Bash trap needs a background child](project_bash_trap_needs_background_child.md): a TERM/INT trap fires only after the foreground command returns: background, record the pid, wait, kill by pid in the trap; never `kill -- -$$`
...
- [Visible surfaces need consent](feedback_visible_surfaces_need_consent.md): hooks install silently, but any user-visible surface (status line, tab titles, banners) needs explicit revocable consent: prompt if interactive, else print the enable command
...
- [CS_CLAUDE_SESSION_ID is the launch id](project_cs_claude_session_id_is_launch_id.md): stale after the first rotation; the current conversation is claude_session_id in .cs/local/state; the staleness is load-bearing for crash recovery, never "fix" it
...
- [Full gate runs on ghost](feedback_full_gate_runs_on_ghost.md): EVERY full suite goes to ghost via claude-tmux remote-tests, mods-only is not an exemption (repeated 2026-09-16); `--changed` locally is only the edit loop
...
- [No base-URL gateway](project_no_base_url_gateway.md): do NOT route cs through an ANTHROPIC_BASE_URL gateway: Claude Code sends the subscription OAuth token to a custom base URL and cs's rewriter child keeps that var (2026-09-06)
...
- [tmux pane env wall](project_tmux_pane_env_wall.md): panes inherit no provider API keys; `tmux new-window -e` forwards one (the logger records it unexpanded) but the value still reaches tmux's argv, so it is not a never-in-argv guarantee
