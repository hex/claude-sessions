...
- [Design discussion style](feedback_design_discussion_style.md): open-ended design → prose + ELI5 iteration, not AskUserQuestion menus; AskUserQuestion is fine for concrete either/or
...
- [pipefail SIGPIPE early-exit class](project_pipefail_sigpipe.md): early-exiting consumers (grep -q, sed q) silently kill cs at exit 141 on >64KB input; read files directly, probe at >64KB
...
- [Claude Code bundle is the authoritative contract](reference_claude_code_bundle.md): grep the installed bundle (~/.local/share/claude/versions/<v>), not public docs, for hook/settings surfaces
- [No foreground sleep in dispatched agents](project_subagent_no_foreground_sleep.md): the harness blocks it and a polling reviewer hangs for good; every long-work dispatch must forbid it
...
- [Skill surfaces over cs verbs](user_prefers_skill_surfaces.md): RULE: fewest cs verbs; every capability reachable as a skill/plugin/command; a verb is only plumbing
...
- [Statusline memo inheritance trap](project_statusline_memo_inheritance.md): an inherited env var of the same name seeds a bare global memo; ready-flag, reset at main(), 10#-normalize
...
- [Hold for adversarial review](feedback_hold_for_adversarial_review.md): Alex holds for an independent Fable review even with all gates green, NOT only security work; wait for it on any release
- [Subagent idle is not dead](project_subagent_idle_not_dead.md): never judge by mtime or an unanswered ping; NAMED agents are tmux-backed; past a time box re-dispatch UNNAMED or codex exec
- [SessionEnd source asymmetry](project_sessionend_source_asymmetry.md): a `/clear` ends as `user_exit`, not `clear`; only SessionStart tells rotation from quit, so lineage logic lives there
- [/code-review lead never reports](project_code_review_skill_is_pr_shaped.md): takes a range, spawns 8 finders whose JSON lands in your inbox (16 KB truncation), lead parks forever
...
- [Secret-shaped fixtures vs push protection](project_secret_fixtures_push_protection.md): GH013 rejects a literal credential pattern even in a fixture; assemble tokens at runtime, never unblock
...
- [Mail `from` is empty, not null](project_mail_from_empty_string.md): `cs -msg` writes `from:""` outside a cs session so jq's `//` never fires; consumers must test emptiness explicitly
...
- [commands/ subdir renames, skills/ subdir hides](project_claude_dirs_namespace_semantics.md): measured on 2.1.258; keep cs commands and skills flat; probe via `env -u ANTHROPIC_API_KEY claude -p`
...
- [Bash trap needs a background child](project_bash_trap_needs_background_child.md): a TERM/INT trap waits for the foreground command; background, wait, kill by recorded pid; never `kill -- -$$`
...
- [Transcript usage fields](reference_transcript_usage_fields.md): dedup by requestId (else ~2.9x); costUSD is 0 on subscription; statusline stdin carries rate_limits.{five_hour,seven_day}
...
- [Eradicate a shared failure class](feedback_eradicate_shared_failure_class.md): Alex picks fix-everywhere over match-the-old-convention when a review exposes a repo-wide silent-failure idiom
...
- [CS_CLAUDE_SESSION_ID is the launch id](project_cs_claude_session_id_is_launch_id.md): stale after the first rotation; read claude_session_id in .cs/local/state; keep the staleness for crash recovery
...
- [teammate-message frame is not a discriminator](project_transcript_teammate_frame.md): leads carry it once reports arrive; key on the first type:user line; re-measure after the feature runs
...
- [pane_current_command is the version](project_pane_current_command_is_version.md): Claude Code panes report `2.1.263`, not `claude`; never kill by predicate, only by pane ids captured at creation
...
- [Codex review briefs](project_codex_readonly_no_probes.md): "do not edit files" means no fixtures, so grant a scratch dir; on a dirty tree pin the scope or it asks
