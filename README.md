...
- **Conversation rotation** - a heavy conversation can hand off to a fresh one without losing context: the `rotate` skill (self-invoked, or nudged once per conversation at 65% context) writes a lineage-stamped handoff to `.cs/handoffs/` and arms it, then `/clear` continues from it without leaving Claude Code — and continues on its own, since the session wakes itself a moment later and starts the handoff's next step with nothing typed (`CS_NO_ROTATION_WAKE=1` to wait for a word instead). Exiting and answering `r` at the next `cs <name>` launch does the same; `d` discards the handoff. `cs -conversations` shows the resulting chain. The `cs-rotate` mod, a Claude Code function-hooks plugin the installer deploys and every cs launch enables, puts a `1: rotate this conversation` button above the prompt once context reaches 40% (`CS_ROTATE_BUTTON_CTX` moves it), beside `2: wrap up this session`, which only arms itself, since a wrap runs two Opus passes and replaces the summary: for five seconds `3: yes, run /wrap` appears beside it, and `3` runs it; the rotate press runs `/rotate`, and once the handoff is armed the band draws `1: /clear and continue from the handoff` alone and runs the `/clear` itself. `CS_ROTATE_FORCE_CTX=<percent>` makes the mod press the button for you: a turn ending past that percentage runs `/rotate`, and once the handoff is armed the capsule counts down 20 seconds to the `/clear` (press `1` to go now, send a prompt to stop it). `CS_NO_FUNCTION_HOOKS=1` withholds the mod; see [docs/hooks.md](docs/hooks.md).
- **A hint line that knows the session** - the `cs-hint` mod draws one dim line under the prompt in the lead conversation of a cs session, while the composer is empty and idle: unread mail with its latest sender, an armed handoff and the `/clear` that continues it, queued tasks with the gate's state, the purpose of the handoff this conversation continues until your first prompt, and otherwise one cs tip that fits the session (`/finish` in a worktree, `/feature` elsewhere), stepping once per turn. At most two facts on the line. `CS_NO_HINTS=1`, or `hints: off` in `.cs/local/state`, leaves the engine's line alone; see [docs/hooks.md](docs/hooks.md).
...
- **Second opinion at a decision point** - When [claude-council](https://github.com/hex/claude-council) or the codex plugin is present, the Stop hook appends a standing note naming what each one offers: `/claude-council:advise`, which puts the conversation itself to external models instead of a question you retype, and `/codex:review` before you call built work done. Claude judges whether a moment qualifies. It checks with you before running the council, because the digest goes to third-party providers, and it only offers `/codex:review`, which is yours to type. One note whatever you have installed, at most once every 30 minutes, and silent when you have neither. See [docs/hooks.md](docs/hooks.md)
...
- **Health checks** - `cs -doctor` reports status of Keychain backend, hook registration, shadow-ref freshness, auto-memory writability, status line registration, Claude Code settings audit (hooks/MCPs/permissions/env vars counts), cumulative token usage for the current project, whether the cs-rotate and cs-hint mods ran, and an authority section listing every hook that injects into the model's context together with the switch that turns each one off
...
- Adds `/summary`, `/checkpoint`, `/sweep`, and `/wrap` commands, the `store-secret`, `prose-hygiene`, `rotate`, `finish`, `feature`, and `write-as-me` skills, and the `cs-rotate` and `cs-hint` mods to `~/.claude/`
...
- **Narrative** (`.cs/memory/narrative.<actor>.md`) — A per-actor lab notebook for findings, observations, and ideas during a session. Each co-developer writes their own file (so shared sessions never conflict) and reads it in full on resume. A teammate's file you only read from the line the resume digest names, never the whole thing. The notebook is append-only — a disproven entry gets a dated correction, never an edit — and `cs -narrative rotate` (run by `/wrap`, by the session manager's **Rotate narrative** entry, flagged at conversation start, by the Stop hook and by `cs -doctor`; `cs <name> -narrative rotate` reaches a session you are not in) moves the oldest sections verbatim into `.cs/narrative-archive/<actor>/` once a file passes `CS_NARRATIVE_MAX_BYTES` (224 KiB, under the Read tool's 256 KiB ceiling), keeping a `CS_NARRATIVE_KEEP_BYTES` (112 KiB) tail. Stored as native Claude Code memory files; see [docs/session-layout.md](docs/session-layout.md) for how that works.
...
the nudge threshold, where rotation takes over). At 65% context, the same
