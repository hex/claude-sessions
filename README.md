...
- **Auto-memory bucket guidance** - Each session's CLAUDE.md includes a signal-phrase decision table that maps user statements to the right `.cs/memory/<bucket>_*.md` file (user, feedback, project, reference) with dedup, lazy-load, and "never invent" discipline. The block is wrapped in a `<!-- cs:memory-rules -->` sentinel; users can opt out by deleting the content and keeping the sentinel as a tombstone. Legacy CLAUDE.md files are lazily backfilled via `migrate_session` Phase 9.
