...
- **Deterministic Claude-session resume** - Each session pre-allocates a UUID at creation and records it in `.cs/README.md` frontmatter; `cs <name>` resumes via `claude --resume <uuid>` (the exact conversation) rather than `--continue` (the most-recent claude conversation, which can be a sibling). Hooks read `$CS_CLAUDE_SESSION_ID`; `cs -doctor` cross-checks the recorded UUID against the live `$CLAUDE_CODE_SESSION_ID`. A `ps`-based guard refuses to spawn a second claude for the same UUID (`--force` overrides). Legacy sessions bind to their existing claude transcript on next launch via `migrate_session` Phase 8 — the recorded UUID is discovered from `~/.claude/projects/<encoded-cwd>/` rather than minted blind, and an orphaned recorded UUID (one with no matching transcript file) is self-healed to the most-recent real transcript. Declining the resume prompt (`N`) rebinds the session to a fresh UUID and passes `--session-id <new>` to claude so the fresh conversation stays tracked, with `CS_FRESH_REBIND=1` signalling SessionStart hooks to inject a "clean break, lazy-read .cs/ for context" notice instead of acting like a cold-start.
- **Auto-memory bucket guidance** - Each session's CLAUDE.md ships an imperative action-sequence (Read → check → Write → announce) telling claude how to capture durable facts continuously into `.cs/memory/<bucket>_*.md` files (user, feedback, project, reference). Includes signal-phrase routing tables, lazy-load read signals, and non-negotiable guardrails (durable only, dedup, never invent, one bucket per fact, MEMORY.md index in sync). The block is wrapped in a `<!-- cs:memory-rules -->` sentinel; users can opt out by deleting the content and keeping the sentinel as a tombstone. Block content is generated by a single `_emit_memory_rules_block` helper consumed by both `write_session_claude_md` and `migrate_session` Phase 9 (no HEREDOC drift between new-session and lazy-backfill paths). Smart Phase 9 auto-upgrades existing sessions on next launch — detects the old v2026.5.2 block via header fingerprint and replaces it in place; tombstone opt-outs are preserved; user customizations of the block content are clobbered (documented trade-off).
