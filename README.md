...
- **Health checks** - `cs -doctor` reports status of Keychain backend, hook registration, shadow-ref freshness, auto-memory writability, status line registration, Claude Code settings audit (hooks/MCPs/permissions/env vars counts), cumulative token usage for the current project, whether the cs-rotate mod ran, and an authority section listing every hook that injects into the model's context together with the switch that turns each one off
...
- Adds `/summary`, `/checkpoint`, `/sweep`, and `/wrap` commands, the `store-secret`, `prose-hygiene`, `rotate`, `finish`, `feature`, and `write-as-me` skills, and the `cs-rotate` mod to `~/.claude/`
