...
- **Status line** - `cs-statusline` renders Claude Code's status bar as one line of squared, abutting pills: a Claude logo badge (Claude coral), session name tinted with the session's `claude_session_color` (Claude Code's exact `/color` palette, matched to the terminal tab), git branch (bold slate-blue accent) with ahead/behind and dirty counts, model + effort level, context %, 5-hour/weekly rate limits (the 5-hour block also shows the time until the window resets), and session cost. Everything comes from the status-line stdin JSON plus one bounded git call (`GIT_OPTIONAL_LOCKS=0`, 2s timeout) and one small `.cs/` read; no transcript parsing, no network, no writes. `install.sh` only registers it in `settings.json` with confirmation (and never replaces an existing status line without asking); enable or remove it any time with `cs -statusline enable|disable`. Choose and order segments with `CS_STATUSLINE_SEGMENTS`, disable with `CS_STATUSLINE_DISABLE=1`, tune thresholds with `CS_STATUSLINE_CTX_WARN`/`CS_STATUSLINE_CTX_CRIT`; outside cs sessions the cs-only segments go blank. cs detects the terminal's light/dark theme at launch via an OSC 11 background query (inside tmux the query is sent through DCS passthrough so it reaches the real outer terminal; requires `allow-passthrough on`, falling back to the macOS appearance otherwise) and exports it as `CS_TERM_THEME`, which also acts as a manual override; `cs -detect-theme` shows the result. In truecolor mode the bar also stretches to the full terminal width, fading its trailing edge into the real background color captured by that same OSC 11 query (exported as `CS_TERM_BG_RGB`) — needs Claude Code ≥ 2.1.153 (for `$COLUMNS`) and a successful OSC 11 query; missing either just renders the bar as it would without this feature. See [docs/statusline.md](docs/statusline.md)
