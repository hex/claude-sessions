...
- **Second opinion at a decision point** - When the [claude-council](https://github.com/hex/claude-council) plugin is present, the Stop hook appends a standing note naming `/claude-council:advise`, which puts the conversation itself to external models rather than a question you retype. Claude judges whether a moment qualifies; the note asks it to check with you first, because the digest goes to third-party providers. At most once every 30 minutes, and silent when the plugin is absent. See [docs/hooks.md](docs/hooks.md)
...
- **Health checks** - `cs -doctor` reports status of Keychain backend, hook registration, shadow-ref freshness, auto-memory writability, status line registration, Claude Code settings audit (hooks/MCPs/permissions/env vars counts), cumulative token usage for the current project, and an authority section listing every hook that injects into the model's context together with the switch that turns each one off
- **Usage attribution** - `cs -usage` shows which sessions are consuming the 5-hour and weekly rate-limit windows: per-session input/output token sums (deduplicated by API request, cache-read excluded), anchored at the true reset boundaries when the cs status line is active. `cs -usage <name>` breaks one session down per conversation with a lifetime column. Both views fold in every subagent and workflow-agent transcript beneath a conversation, however deep Claude Code nests them, while the model column stays the conversation's own. A `READS>350L` column counts Read tool results over 350 lines as `untargeted/all ~tokens`: a read is targeted when the model asked for a slice (an offset or a limit), and the token figure is the untargeted characters at four per token, the share a size gate could have intercepted. Reads made through Bash (`cat`, `head`) carry no file shape in the transcript and are not counted.
...
- **Narrative** (`.cs/memory/narrative.<actor>.md`) — A per-actor lab notebook for findings, observations, and ideas during a session. Each co-developer writes their own file (so shared sessions never conflict) and reads it in full on resume. A teammate's file you only read from the line the resume digest names, never the whole thing. The notebook is append-only — a disproven entry gets a dated correction, never an edit — and `cs -narrative rotate` (run by `/wrap`, by the session manager's **Rotate narrative** entry, flagged by the Stop hook and `cs -doctor`; `cs <name> -narrative rotate` reaches a session you are not in) moves the oldest sections verbatim into `.cs/narrative-archive/<actor>/` once a file passes `CS_NARRATIVE_MAX_BYTES` (512 KiB), keeping a `CS_NARRATIVE_KEEP_BYTES` (256 KiB) tail. Stored as native Claude Code memory files; see [docs/session-layout.md](docs/session-layout.md) for how that works.
