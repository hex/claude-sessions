...
- **Finished-turn notification** - on macOS with [terminal-notifier](https://github.com/julienXX/terminal-notifier) installed (`brew install terminal-notifier`), a turn that ends while the terminal is not the frontmost app posts a notification with the cs owl as its icon; clicking it brings the terminal forward, and your next prompt takes it down. `CS_NO_NOTIFY=1` disables.
