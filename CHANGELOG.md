...
- The hint line under the prompt says what a session is waiting on. `mods/cs-hint/` is a second Claude Code mod, deployed, enabled and reported like cs-rotate (installer, the function-hooks flag, its own heartbeat at `.cs/local/cs-hint.heartbeat` for `cs -doctor`, removed by `cs -uninstall`). In the lead conversation of a cs session, while the composer is empty and idle, the dim `? for shortcuts` line reads instead, in priority order and at most two at once: unread mail (`2 messages from <session> · cs -msg`, counted from `.cs/local/mail/new/`), an armed handoff (`handoff armed · /clear continues it`, by the SessionStart hook's own rule, so it shows below cs-rotate's context threshold too), queued tasks with the gate's state (`3 queued · gate waiting`, `deferred` inside the hook's ten-minute cooldown, `draining` while a drain runs; an empty queue says nothing), and the purpose of the handoff this conversation continues (`continuing: <purpose>`, until your first prompt; the rotation's own wake does not end it). With nothing waiting it shows one tip from a fixed list of cs surfaces: the first fits the session (`/finish` in a worktree session, `cs -statusline caps` on a Fable model, `/feature` elsewhere), the rest step once per finished turn and never on a timer. Once drawn the line refreshes every five seconds, since mail, the queue and the marker change from outside the process. While you type, while a turn runs, in a teammate claude, outside a cs session, with `CS_NO_HINTS=1` in the launching shell or `hints: off` in `.cs/local/state`, the engine's line is untouched. The doctor row for a mod is one function now, run once per mod.
