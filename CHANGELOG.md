...
### Features
- A second key on the rotation band wraps the session. Where the band draws `1: rotate this conversation` it now draws `2: wrap up this session` beside it; the key runs `/wrap` (memory distillation, the session summary, the narrative rotation). Since a wrap replaces `.cs/summary.md` and runs three Opus passes, one press only arms the key, `2: press 2 again to /wrap` for five seconds, and the second press within that window runs it; a prompt or a `/clear` meanwhile disarms it, and a run the engine refuses shows a toast. Once a handoff is armed the band draws the `/clear` key alone.
