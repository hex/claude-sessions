...
### Changed
- The rotation band asks instead of counting down, and leaves the percentage to the status bar. Past the forced threshold (`CS_ROTATE_FORCE_CTX`), once the `rotate` skill had armed its handoff the band used to add `/clear in 20s`, count down once a second and run the `/clear` itself at zero; it now opens Claude Code's own AskUserQuestion dialog from a timer — `The handoff is written. Clear now and continue from it?`, with `Clear and continue` and `Not yet` — and only the yes clears. The question is asked once per conversation whatever the answer, and the handoff and the lead are re-read before the `/clear` runs, since the dialog can stand open for as long as you like; a dismissal, and a `-p` run with nobody to ask, leave the band's key where it is. The wrap key asks the same way: `2` opened a five-second window and put the confirmation on `3`, and now opens `Run /wrap for this session?` on the press, so one key reaches the question. The band itself is a bare line with a blank line above it on the status bar's own capsule fill (a shade of `CS_TERM_BG_RGB` a tenth away from itself, and no fill at all where cs measured no background), and it no longer repeats the context percentage the bar already carries.
