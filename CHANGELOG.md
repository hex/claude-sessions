...
### Fixes
- `/finish` no longer re-runs the gate on a PR that origin's CI already landed. On the remote path, when the base has nothing origin lacks, the temp merge is a fast-forward onto the PR's merge commit, a tree CI ran; cs now skips the gate there and the report says `gate skipped`. A base with its own commits makes a merge CI never saw, and the gate still runs on that. A red gate also says what the temp is: a clean checkout of the merged commit, without the project's gitignored dependencies, `.env` or build output, so a gate that needs them must create them first (`-- sh -c 'npm ci && npm test'`), and the skill asks once about that install step when a project has a lockfile or `.env.example` and no instructions.
