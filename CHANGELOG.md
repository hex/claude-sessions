...
### Fixes
- The status line's `~/.cache/cs/tmux-client/` and `~/.cache/cs/tmux-real/` no longer grow without bound. Both take the Claude Code pid as their key, so every conversation left two files behind and nothing removed them (about 4,000 each after a month). The Fable usage refresher, which already runs detached and under its own lock, now deletes entries older than an hour; the once-a-second render still forks nothing for it.
