...
### Changed
- The 5h rate-limit capsule is always on the bar, and the coarse windows (wk, and fable on a Fable session) join it at 50% instead of 70%, in the fixed order 5h, wk, fable so capsules never swap places. 5h and wk share one quota capsule, joined by a bar; the Fable window keeps its own. One hot window inverts the shared capsule whole, since a red quota capsule means throttling is near whichever window tripped. Reveal and colour are now separate: every window is neutral below 70, amber from 70, red from 90, and the reset countdown still joins 5h at 70 and the coarse windows at 80.
