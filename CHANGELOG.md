...
### Fixes
- `cs -narrative rotate` now commits in a repo that ignores `.cs/` but tracks the narrative with `git add -f`. Git refused to stage any path under the ignored directory, the tracked narrative included, so cs wrote the rotation but never committed it. A narrative git does not track is still left out of the commit.
