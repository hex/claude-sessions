...
### Removed
- The finished-turn macOS notification. cs no longer posts anything to Notification Centre: the Stop hook's poster, the two hooks that took the notification down, the `cs.app` sender bundle the installer assembled under `~/.local/share/cs/`, the owl icon that shipped beside the hooks, the doctor row and `CS_NO_NOTIFY` are all gone. A hook can only ask whether the terminal application is frontmost, so it stayed silent whenever you were in another tab of the same window, and it knew nothing of your other sessions, so each one posted for itself. That belongs to something that can see every session at once, which is the iTerm2 sidebar, not cs. An install from before this version leaves `~/.local/share/cs/cs.app` behind; delete it by hand. `cs -msg --kind notify` and the queue's notification inbox are unrelated and unchanged, as is the iTerm2 dock bounce.
...
### Features
- A finished turn reaches you in another window. On macOS with `terminal-notifier` installed, the Stop hook posts a notification (`cs: <session>`, "finished a turn", the cs owl as its icon) when the terminal is not the frontmost app; clicking it brings the terminal forward, and your next prompt, or the next conversation's start, takes it down. The owl is the sender's icon: macOS ignores any icon a command-line poster names, so the installer assembles `cs.app` under `~/.local/share/cs/` from the `terminal-notifier.app` Homebrew ships, with the owl as its icon and cs's own bundle id (macOS asks once whether cs may notify), and the hooks post and remove through it; without the bundle they use `terminal-notifier` from PATH with its stock icon. The terminal's identity comes from `__CFBundleIdentifier`, so it works inside tmux, where `TERM_PROGRAM` reads `tmux`; when the frontmost app or the terminal cannot be told, nothing is posted. Only the launched conversation posts or removes one, never a tmux teammate. `CS_NO_NOTIFY=1` disables; `cs -doctor` reports the poster and a bundle a `brew upgrade` left behind; `cs -uninstall` removes the bundle. The icon's source ships beside the hooks as `cs-logo.png`, and the repository carries the logo itself at `assets/logo.png`.
