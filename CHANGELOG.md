...
### Fixes
- `cs -doctor` no longer warns "autosave may be broken" after a `/clear`. It looked for the snapshot ref of the conversation that started the session, which the `/clear` had already deleted, instead of the current conversation's. It now reads the current id from `.cs/local/state`. It reports a missing ref as "no snapshot for this conversation yet": autosave runs only on Edit and Write, so a conversation that has only used Bash has none.
