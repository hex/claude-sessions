...
### Changed
- The rotation band stops offering a wrap that already finished. `/wrap` gains a last pass that writes `.cs/local/wrapped` with the lead conversation's id once memory, summary and narrative rotation are done, and `2: wrap up this session` hides while that marker names the conversation, however the wrap was started; the next prompt empties it and brings the key back. A standalone `/summary` does not hide the key, and a wrap that fails a pass writes no marker.
