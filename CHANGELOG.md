...
- The scope hook keeps its scan on a shell whose clock ticks in whole seconds. On a bash without `$EPOCHREALTIME` the elapsed reading jumps a full 1000 ms at the first tick however little was spent, and the deadline is `-ge`, so any `CS_SCOPE_BUDGET_MS` at or under a tick dropped the grounded scan on that boundary alone. A positive sub-second budget now takes the 1500 ms default, the only value above one tick; zero still skips every prompt, and the stage trace names the budget the run settled on.
- A `Retry-After` header too long for the shell's arithmetic no longer ends usage polling. Twenty digits wrapped past 64 bits to a positive number centuries away, and the refresher would have waited there for good; a header of more than fifteen digits now takes the 600 s floor, like one that is not a number. `CS_STATUSLINE_NOW` is bounded the same way.
- `cs -spawn` stages nothing when the seed cannot be written. The seed write was an AND list, which `set -e` does not judge, so a failed write still opened the window, and the brief it had already staged stayed behind for the next spawn of that name to inherit although that spawn asked for no brief. The failure now aborts and removes the staged brief.
