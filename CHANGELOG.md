...
### Features
- One key rotates a conversation. `mods/cs-rotate/` is an opt-in Claude Code mod (function hooks, TypeScript inside Claude Code's process): once context reaches the crit band the band above the prompt draws `1: rotate this conversation`, and pressing `1` fills the composer with `/rotate ` for the purpose line. Nothing is sent and no command runs on its own; the button hides while a turn runs, and only the lead conversation of a cs session gets one (a teammate claude or a plain conversation draws nothing). `cs -doctor` reports the mod by the heartbeat it writes at session start, not by its directory. Enabling it is two lines on one machine (see docs/hooks.md); `install.sh` leaves it alone while Claude Code keeps function hooks behind `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1`.
