...
_mail_reply_peer() {  # maildir, thread -> "peer<TAB>parent id"; rc 1 if unknown
    local maildir="$1" id="$2" f newest=""
...
_mail_send() {  # target, [--kind|-k KIND] [--reply THREAD] body
    ...
    if [ -n "$reply_thread" ]; then
        ...
        local pair derived
        pair=$(_mail_reply_peer "$CLAUDE_SESSION_META_DIR/local/mail" "$reply_thread") \
