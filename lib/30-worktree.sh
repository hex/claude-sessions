...
$wt_untracked"
    ...
    if [ "$mode" = "ignored" ]; then
        aside=$(mktemp -d "${TMPDIR:-/tmp}/cs-retire.XXXXXX")
        mv "$wt_dir/.cs" "$aside/cs" || error "Could not set aside $wt_dir/.cs for the record fuse"
    fi
    local remove_err
    if ! remove_err=$(git -C "$base_dir" worktree remove --force "$wt_dir" 2>&1); then
        [ -n "$aside" ] && mv "$aside/cs" "$wt_dir/.cs" 2>/dev/null && rmdir "$aside" 2>/dev/null
