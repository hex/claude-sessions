...
merge_worktree_session() {
    ...
    # cs writes its own bookkeeping into every worktree — the whole .cs tree in
    # ignored mode, plus the settings and protocol files — and none of it is the
    # user's to commit. Ignored-mode fusion carries .cs into the base before the
    # removal below, and the removal already passes --force precisely because
    # these are expected. Counting them as dirt refuses a merge over the files
    # cs itself just created, in any project whose .gitignore does not happen to
    # name them. Everything else still refuses: the point of the gate is work
    # the user would lose.
    local wt_untracked
    wt_untracked=$(git -C "$wt_dir" ls-files --others --exclude-standard 2>/dev/null \
