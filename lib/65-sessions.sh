...
# Print every session name, one per line, as completion candidates.
# A .cs/ marker is what makes a directory a session; SESSIONS_ROOT also holds
# unrelated directories such as editor config. Symlinks count: `cs -adopt` links
# repos that live elsewhere on disk into SESSIONS_ROOT, and `-d` resolves through
# the link. Kept free of git and keychain lookups so a TAB press stays fast.
complete_sessions() {
