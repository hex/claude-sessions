...
_export_term_theme() {
    ...
    # children) render with the right palette. CS_TERM_THEME_AUTO is a bare
    # presence marker: it flags that CS_TERM_THEME came from launch auto-detection
    # rather than an explicit user pin, so the statusline knows it may override
    # the launch theme with the live OS appearance (macOS auto dark mode). A
    # user-set CS_TERM_THEME returned above and carries no marker, so it stays a
...
# with the macOS branch of _sl_detect_theme in bin/cs-statusline, which reads the
# same key live per render (the statusline is standalone and sources nothing).
