...
2. **Command namespacing.** Plugin commands may surface as `/claude-sessions:summary` when names collide. Verify whether `/summary`, `/wrap`, `/sweep`, `/checkpoint`, `/compact-discoveries` keep their bare names when unique, and whether the CLAUDE.md wrap-cues prose (which names `/wrap` etc.) needs the qualified forms.
