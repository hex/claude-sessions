...
# On macOS the installer assembles ~/.local/share/cs/cs.app from the
# terminal-notifier.app Homebrew ships: the binary copied, the owl built into
# an icns from hooks/cs-logo.png, the bundle id and name cs's own, an ad-hoc
# signature. The notification then carries the owl as its sender's icon, the
# one place macOS lets a command-line poster choose an icon. The source app is
# a fake here (CS_NOTIFIER_SRC_APP), so the test needs only the macOS tools
# that build the icns and sign; elsewhere it is skipped.
_fake_notifier_src() {  # dir
    mkdir -p "$1/Contents/MacOS" "$1/Contents/Resources"
    # A real Mach-O (codesign seals the main executable), one that exits 0
    # for the installer's -help smoke run.
    cp /usr/bin/true "$1/Contents/MacOS/terminal-notifier"
    : > "$1/Contents/Resources/Terminal.icns"
...
test_install_assembles_the_notifier_bundle_on_macos() {
    if [ "$(uname -s)" != "Darwin" ] || ! command -v iconutil >/dev/null 2>&1; then
        echo "    SKIP (macOS with iconutil only)"
        return 0
    fi
    local fake_home="$TEST_TMPDIR/owl-home" src="$TEST_TMPDIR/src.app"
    mkdir -p "$fake_home/.claude"
    _fake_notifier_src "$src"
    HOME="$fake_home" CS_NOTIFIER_SRC_APP="$src" bash "$INSTALL_SH" > "$TEST_TMPDIR/install.out" 2>&1 < /dev/null \
        || { echo "  FAIL: install.sh exited non-zero"; return 1; }
    local app="$fake_home/.local/share/cs/cs.app"
    [ -x "$app/Contents/MacOS/terminal-notifier" ] \
        || { echo "  FAIL: the notifier binary was not copied into the bundle"; return 1; }
    # Signing rewrites the copy, so the source is identified by its digest.
    assert_eq "$(shasum -a 256 "$src/Contents/MacOS/terminal-notifier" | cut -d' ' -f1)" \
        "$(cat "$app/Contents/Resources/cs-source.sha256")" "the source digest is recorded" || return 1
    [ -s "$app/Contents/Resources/Terminal.icns" ] \
        || { echo "  FAIL: the owl icns is missing or empty"; return 1; }
    # The icns is a fresh build from the 256 px png, not the 1.4 MB one a full
    # 1024 px set would make: sizes stop at 256.
    [ "$(wc -c < "$app/Contents/Resources/Terminal.icns")" -lt 400000 ] \
        || { echo "  FAIL: icns larger than a <=256 px set: $(wc -c < "$app/Contents/Resources/Terminal.icns") bytes"; return 1; }
    assert_eq "com.hex.cs.notifier" "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleIdentifier' "$app/Contents/Info.plist")" \
        "the bundle id is cs's own" || return 1
    assert_eq "cs" "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleName' "$app/Contents/Info.plist")" \
        "the bundle name is cs" || return 1
    codesign --verify "$app" 2>/dev/null || { echo "  FAIL: the assembled bundle is not signed"; return 1; }
    assert_file_contains "$TEST_TMPDIR/install.out" "notifier bundle" "the installer reports the bundle" || return 1

    # A second install rebuilds in place: a stale binary is replaced, nothing
    # is left beside it.
    echo 'stale' > "$app/Contents/Resources/cs-source.sha256"
    touch "$app/Contents/leftover"
    HOME="$fake_home" CS_NOTIFIER_SRC_APP="$src" bash "$INSTALL_SH" > /dev/null 2>&1 < /dev/null \
        || { echo "  FAIL: second install.sh exited non-zero"; return 1; }
    assert_eq "$(shasum -a 256 "$src/Contents/MacOS/terminal-notifier" | cut -d' ' -f1)" \
        "$(cat "$app/Contents/Resources/cs-source.sha256")" "a stale bundle is rebuilt" || return 1
...
test_install_skips_the_notifier_bundle_without_a_source_app() {
    ...
    mkdir -p "$fake_home/.claude"
    HOME="$fake_home" CS_NOTIFIER_SRC_APP="$TEST_TMPDIR/absent.app" bash "$INSTALL_SH" > "$TEST_TMPDIR/install.out" 2>&1 < /dev/null \
        || { echo "  FAIL: install.sh exited non-zero"; return 1; }
