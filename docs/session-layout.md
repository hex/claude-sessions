...
| `cs-rotate.heartbeat` | UTC timestamp of the last session start at which the cs-rotate mod ran — `cs -doctor` reports the mod from this file, never from the plugin directory being present. |
| `cs-update.heartbeat` | UTC timestamp of the last session start at which the cs-update mod ran, written by the mod's `session.start` when Claude Code loads it — `cs -doctor` reads it the same way it reads `cs-rotate.heartbeat`, never the plugin directory being present. |
| `cs-update.done` | Version and outcome line of the last `cs -update` the pane finished, written by the mod on a clean exit and read back at the next `session.start`: installing the update rewrites the mod's own deployed file, Claude Code reloads it mid-conversation, and the reload drops the module state the pane was drawing from. Cleared (written empty) at the next launch that finds nothing pending for that version. |
| `cs-rotate.forced` | The conversation `CS_ROTATE_FORCE_CTX` already rotated, written before the `/rotate` runs so a failed one is not retried every turn. |
| `wrapped` | The conversation a `/wrap` finished in (its `CLAUDE_CODE_SESSION_ID`), written by its last pass; the cs-rotate band hides the wrap key while it names the conversation, and the next prompt empties it. |
...
| `scope-prompt.trace` | One line per stage of each `scope-prompt` run, appended as that stage finishes — so a run its timeout killed still names the stage it hung on, plus a `budget=<N>` line naming the budget the run settled on. A run that passed `CS_SCOPE_BUDGET_MS` before its scan records a `skip` stage and still exits normally. Opt out with `CS_SCOPE_TRACE_DISABLE=1`. |
...
| `context-pct` | Latest context-window percentage of the launched conversation, stamped by the status line (a tmux teammate sharing the directory only touches the file, so its reading never replaces the lead's); the narrative reminder reads it to suggest compaction, the launch card reads it to say how full the conversation you are resuming already is, and cs-tui uses its mtime as the liveness heartbeat for conversations opened outside cs. A `<file>.at` beside it holds `epoch value` of the last write, so an unchanged reading is rewritten once a minute rather than every render. |
| `limits` | Latest 5-hour/weekly rate-limit readings, stamped by the status line; read by `cs -usage` window anchoring and the queue's rate-limit breaker. A `<file>.at` beside it holds `epoch value` of the last write, so an unchanged reading is rewritten once a minute rather than every render. |
