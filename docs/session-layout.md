...
| `context-pct` | Latest context-window percentage, stamped by the status line; the narrative reminder reads it to suggest compaction, the launch card reads it to say how full the conversation you are resuming already is, and cs-tui uses its mtime as the liveness heartbeat for conversations opened outside cs. |
