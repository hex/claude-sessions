...
| `.cs/handoffs/` | Lineage-stamped conversation handoffs written by the `rotate` skill (parent UUID, purpose, continuation plan). Each carries a `status:` field — `unconsumed` while pending, flipped to `consumed` by the SessionStart that rotates into it, to `discarded` by the resume prompt's `d` answer, or to `superseded` when a later rotation retires it. A later rotation also prunes the spent ones: `consumed`, `discarded` and `superseded` files older than 30 days by `created:` are deleted unless they are among the 10 newest, which is the only path in cs that removes a handoff. | default |
