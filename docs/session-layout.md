...
| `cs-hint.heartbeat` | UTC timestamp of the last session start at which the cs-hint mod ran; `cs -doctor` reports the mod from this file. |
| `cs-hint.spoken` | Records that the hint line already showed `continuing: <purpose>` for the handoff this conversation continues, so a reload does not bring it back. |
...
| `scope-prompt.trace` | One line per stage of each `scope-prompt` run, appended as that stage finishes — so a run its timeout killed still names the stage it hung on. A run that passed `CS_SCOPE_BUDGET_MS` before its scan records a `skip` stage and still exits normally. Opt out with `CS_SCOPE_TRACE_DISABLE=1`. |
