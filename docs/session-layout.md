...
| `cs-rotate.heartbeat` | UTC timestamp of the last session start at which the opt-in cs-rotate mod ran — `cs -doctor` reports the mod from this file, never from the plugin directory being present. |
