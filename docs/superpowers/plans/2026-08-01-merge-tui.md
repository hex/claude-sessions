...
The `case` pattern is anchored with a literal newline on both sides so a path that is a prefix of another registered path cannot match. This avoids piping `registered` into `grep -q`, whose early exit can SIGPIPE the producer.
