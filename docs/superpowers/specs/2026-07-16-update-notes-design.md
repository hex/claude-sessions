...
5. **The renderer** (`professional and cool`, shared by --check and
   ...
   - Inline: `**x**` → bold; `[text](url)` → text only; `` `x` `` →
     `${GOLD}` tint in bullet BODIES (applied per folded line so width
     math sees plain text; a span split across a fold boundary degrades
     to plain untinted text) — in titles, summaries, and prose lines,
     code spans render as plain text; other markdown passes through
     as-is.
