...
The description is the only elastic field. The fixed elements are measured first against the payload's `columns`; the description gets what remains, cut with a single ellipsis, and below a small floor it is dropped entirely — the ctx gauge outranks the tail of a description, because a runaway agent's percentage is the thing worth seeing. Unlike the bar, rows are not self-backgrounded pills: they sit on the terminal background inside Claude Code's own panel, so only foreground colors are used.
