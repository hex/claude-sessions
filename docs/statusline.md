...
|---|---|---|---|
| `logo` | A Claude mark (`✳`) badge; the mark holds amber while Claude has finished and awaits input | `.cs/local/attention` marker (raised by the Stop hook, cleared on the next prompt or session start) | Claude coral `rgb(217,119,87)`, white mark; amber mark while awaiting input. A steady state rather than an animation: Claude Code's TUI re-emits only bold/fg/bg from statusline ANSI (terminal blink is dropped) and stops repainting once idle, so any animation would freeze at an arbitrary frame. Omitted in plain (`NO_COLOR`) mode |
