...
|---|---|---|---|
| `logo` | A Claude mark (`✳`) badge; the mark's color pulses while Claude has finished and awaits input | `.cs/local/attention` marker (raised by the Stop hook, cleared on the next prompt or session start) | Claude coral `rgb(217,119,87)`, white mark; the pulse alternates the mark between chiptext and the darker brandshade by epoch-second parity — Claude Code's TUI re-emits only bold/fg/bg from statusline ANSI (terminal blink is dropped), so its own idle repaints animate the phase. Omitted in plain (`NO_COLOR`) mode |
