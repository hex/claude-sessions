...
cs detects the terminal's light/dark theme once at session launch, while it still owns the tty: an OSC 11 background query classified by BT.709 luminance first, falling back to `COLORFGBG` when the query gets no answer, then to OS appearance when neither says anything. The query outranks the variable because `COLORFGBG` goes stale across theme changes; OSC 11 asks the live terminal. Inside tmux `COLORFGBG` is a stale snapshot of the tmux server's start-time environment, so cs ignores it there. tmux that proxies OSC 11 forwards a plain query to the client terminal, so under `$TMUX` cs asks with a plain query first and takes any non-black answer as the real background; a pure-black reply is tmux's own default and is not trusted. When the plain query yields nothing trustworthy, cs retries wrapped for DCS passthrough (needs `allow-passthrough on`), then falls back to OS appearance (`defaults read -g AppleInterfaceStyle` on macOS; `unknown` elsewhere), which is right whenever the terminal theme follows the system. The launch detection sets the palette for cs's own UI and the TUI picker (exported as `CS_TERM_THEME`), and sets a `CS_TERM_THEME_AUTO` marker so the statusline knows the value came from auto-detection rather than an explicit pin. On macOS the statusline may read the live OS appearance (`defaults read -g AppleInterfaceStyle`) on every render, so it follows a mid-session theme switch — system auto dark mode — that the launch value cannot; it cannot re-run the OSC query from a render, which would race its reply into claude's input stream. That substitution is only honest when the terminal actually follows the system, so launch records the OS appearance alongside the measurement (`CS_TERM_OS_THEME`) and the statusline compares them: when the terminal cs measured disagreed with the OS at that moment, the terminal plainly does not track it — a fixed scheme, or a terminal embedded in an app — and the appearance is never consulted for it again, skipping the probe rather than forking `defaults` once a second to discard the answer. Sessions launched before that marker existed keep the previous behaviour. Off macOS it keeps the launch value. Setting `CS_TERM_THEME=light|dark` yourself is an explicit pin (no auto marker) that wins everywhere, including against live macOS detection — use it when the terminal's theme is decoupled from the system. A session already open when the terminal switches keeps its launch palette on non-macOS until relaunched; on macOS the statusline picks up the switch on its next render. On dark terminals the statusline lifts its neutral grey and softens white text; all other colors are self-backgrounded and theme-independent. Run `cs -detect-theme` to see what launch detection yields.

Every signal above describes the terminal cs was launched from, which is the wrong terminal once a tmux session is re-attached from a different one — the palette then tracks the window cs started in rather than the window you are looking at. Inside tmux, cs arms a `client-attached` hook on its own tmux session (appended, so a hook you already had survives; never global, which would reach sessions cs has nothing to do with). On attach the hook runs `cs -detect-theme --write` in a detached window: that pane has a tty of its own, tmux proxies its OSC 11 query to the client that just attached, and the reply routes back to the pane rather than to the tty claude owns. It records the same triple — theme, RGB, and the OS appearance at that moment — to `.cs/local/term-theme`, which the statusline prefers over the launch environment. tmux asks exactly one client when several are attached to a session, so with a light and a dark terminal on one session the answer describes whichever tmux picked; no single answer is right for two terminals at once.

Only the OSC 11 path ever learns the terminal's actual background RGB — the `COLORFGBG`/OS-appearance fallbacks classify light or dark without it. When OSC 11 succeeds, cs exports that RGB as `CS_TERM_BG_RGB` (e.g. `250;248;242`) alongside `CS_TERM_THEME_AUTO`, which is what the [full-width gradient](#full-width-gradient) fades toward. This RGB stays at its launch value even when the live theme flips on macOS, so the gradient tracks the light/dark class but not the exact new background. Set `CS_TERM_BG_RGB` yourself to override.
