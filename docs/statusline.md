...
Default order: `logo,session,notes,mail,git,model,ctx,limits`. A brand badge opens the bar, then identity (which session, which branch, which model), then the gauges. The `cost` segment ships but is off by default — add it to `CS_STATUSLINE_SEGMENTS` to show it.
