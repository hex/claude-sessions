...
|---|---|---|---|
| `logo` | A Claude mark (`✳`) badge; blinks while Claude has finished and awaits input | `.cs/local/attention` marker (raised by the Stop hook, cleared on the next prompt or session start) | Claude coral `rgb(217,119,87)`, white mark. Omitted in plain (`NO_COLOR`) mode, where there is no background |
