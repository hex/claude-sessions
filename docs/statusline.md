...
| `git` | identity | branch with the arrows and `+N!N` marks, bold ink | — | no `.git` here or above | One `git status --porcelain=v1 -b` call |
...
| `ps` + `awk`, the tmux ancestry walk (is this process really in the tmux server `TMUX` names) | 5 min | `~/.cache/cs/tmux-real/`, by parent pid and `TMUX` | once per conversation, and never on the render path: the parent (Claude Code) is the same for every render, and a process that only inherited `TMUX` has a different parent, so it cannot inherit the verdict. On a miss the render answers real and a detached child does the walk, keyed on the same parent, so the render a second later reads a settled verdict; a `ps` that cannot answer is never cached, and the render after it spawns another child. A wrapper Claude Code runs in place of the script (a status-line bridge, a new process each tick) sets `CS_STATUSLINE_PARENT` to the pid it is itself the child of, and both parent-keyed caches key on that |
...
The git call runs with `GIT_OPTIONAL_LOCKS=0` (no index locking for a read-only query) under a 2-second timeout, and is skipped entirely when neither the workspace nor any directory above it holds a `.git`. The workspace is Claude Code's `workspace.current_dir`, which follows the Bash tool's working directory: a `cd` into a subdirectory of the checkout keeps the branch, since the segment climbs to the nearest `.git` (a directory, or the file a worktree keeps) and caches under that checkout. In a session adopted with `cs -adopt`, or a `<base>@<feature>` worktree, that is the checkout itself, so the branch shown is the project's. The same climb means a session directory without a `.git` of its own, under a sessions root that is itself a git checkout, shows that root's branch and dirty counts. A plain session directory is its own one-commit repo on `main`, and a bar reading `main !2` there is that repo and its dirty `.cs` files, not some project the conversation happens to discuss; adopt the project to see its branch.
...
The walk itself is kept off the render that pays for it. It is the one cost a render meets exactly once — it appears in a conversation's first render and in none after — and the first render is the one Claude Code kills for running too long, which starts the retry loop that leaves the bar blank. So the first render answers real, what every pane cs launches is, and spawns a detached child to walk the tree and leave the verdict for the next render. A foreign environment therefore draws its own terminal's palette for one render, about a second, and is corrected on the one after; a real pane never sees a wrong palette at all. The child walks from the parent rather than from the render, which has exited by then, and the parent's chain to the server is the render's chain without its first hop.

One walk runs at a time. A `ps` that stalls never writes a verdict, so without a guard every render would spawn another walker against it — once a second, per conversation, for as long as the session stays open. The render that spawns a walk marks it in flight under the same parent identity (`~/.cache/cs/tmux-walking/`, ten seconds), and a render that finds a fresh mark draws without spawning. Past the ten seconds one render tries again, so a child that died before clearing its mark wedges nothing. The walker bounds itself as well: it waits five seconds on its `ps`, shorter than the mark, and kills it by the pid recorded at its start, so a stalled walker has ended before a replacement can be spawned. A killed walk is a `ps` that could not answer, and leaves no verdict. The usage refresher (`--refresh-usage`) skips the terminal altogether — it draws nothing, so it runs no walk, reads no theme and asks tmux nothing. The mark costs one file write on a cache miss and nothing on a hit, and never a fork; the child clears it, and the usage refresher sweeps the bucket with the other parent-keyed ones.
