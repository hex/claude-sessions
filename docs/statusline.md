...
| `jq` over the Fable usage record, and `date` for its reset countdown | with the record | `fable.<account>.json.fields` beside it: pct, resets_at, fetched_at, next_poll_at, and the reset as an epoch, one per line | written by the refresher with the JSON, never by a render, so a slow parse of an old record cannot overwrite a newer sidecar; the refresher takes its own schedule from the sidecar too, so one left stale by a killed refresher is past due and the next refresh rewrites both; a record with no sidecar (from before it existed) is parsed with `jq` until the next refresh |
