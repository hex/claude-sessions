...
- Inside tmux, releases this pane's `@cs_session` and renames the window after the cs sessions still running in its other panes; with none left the window names itself again (`automatic-rename on`). A `/clear` ends here too, and its SessionStart claims the pane back
