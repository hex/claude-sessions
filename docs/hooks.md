...
- When the launch's `r` answer left a `.cs/local/pending-handoff` marker: consumes it — flips the named `.cs/handoffs/` file's frontmatter to `consumed` (recording the new conversation UUID), removes the marker, and injects a Conversation Rotation preamble pointing Claude at the handoff to read first (mutually exclusive with the Fresh Conversation notice above, since a rotation is also a fresh rebind). A stale marker (handoff file already gone) is removed silently
...
- On all sources: surfaces the cross-session mail digest (same recipe as scope-prompt.sh: unseen `mail/inbox.jsonl` lines, notify bodies inline capped at three, other kinds as per-sender counts pointing at `cs -msg`)
...
- Rotation nudge: when context is at or above `CS_ROTATE_NUDGE_CTX` (default 80, non-numeric override falls back to the default) per the statusline's `context-pct`, and this conversation hasn't already been nudged (`.cs/local/rotate-nudged`, keyed on conversation UUID), emits a one-time `block` suggesting the `rotate` skill to write a handoff into `.cs/handoffs/`. Only reached when the queue isn't gating or draining above — an armed/draining queue owns the turn loop
...
**Queue digest.** Surfaces unseen entries from the per-machine inbox (`.cs/local/notifications.jsonl`) at most once: counts by event plus the last breaker reason, if any, e.g. `cs queue while you were away: 4 task(s) done; breaker tripped: context (91 >= 85), 2 remaining. Run cs -queue log for detail.` The `.cs/local/notifications.seen` cursor advances immediately after building the digest, even when the digest text itself is empty, so surfacing stays at-most-once regardless of what else this prompt does. When scope grounding below also has content, the digest is spliced above the `## Scope (auto-grounded)` block rather than replacing it — every exit path in this hook still delivers a pending digest. Both hooks also surface the cross-session mail digest: unseen `mail/inbox.jsonl` lines become a few context lines (notify bodies inline, capped at three; other kinds as per-sender counts pointing at `cs -msg`). Cursors advance to a pre-counted line total, so torn or mid-write lines are never skipped.
