...
- On all sources: clears the statusline's attention marker (`.cs/local/attention`) — a fresh session is attended by definition — and, inside iTerm2 with shell integration installed, cancels any dock bounce the previous conversation left running (`CS_NO_ITERM2=1` disables); on macOS, for the launched conversation only, also removes the finished-turn notification the previous conversation's last Stop may have posted (`CS_NO_NOTIFY=1` disables)
...
- On macOS, for the launched conversation only, posts a notification (`cs: <session>`, "finished a turn") when the terminal is not the frontmost app, so a turn that ends while the user is in another window reaches them; clicking it brings the terminal forward. Frontmost is read from `lsappinfo`; the terminal's identity from `__CFBundleIdentifier`, which LaunchServices sets on every process a GUI app started and tmux passes through (`TERM_PROGRAM` reads `tmux` there), with `TERM_PROGRAM` (`iTerm.app`, `Apple_Terminal`) as the fallback. When either side is unknown nothing is posted rather than a guess. One notification per session (group `cs:<session>`), so a repeat replaces the last one; scope-prompt.sh removes it at the next prompt and session-start.sh at launch. The poster is `cs.app` under `~/.local/share/cs/` (`XDG_DATA_HOME`), which the installer assembles from the `terminal-notifier.app` Homebrew ships with the cs owl as its icon: macOS shows a notification with its sender's icon and ignores any icon a poster names, so the owl needs a sender of its own. Without the bundle the hooks post through `terminal-notifier` on PATH with its stock icon; without that, nothing. All three hooks resolve the poster the same way (`cs_notifier_bin` in `cs-shared.sh`), because a notification group belongs to the app that posted it. `cs-logo.png` ships beside the hooks as the icon's source. `CS_NO_NOTIFY=1` disables; `cs -doctor` reports the poster (`brew install terminal-notifier`, then `./install.sh`)
...
Runs before each user prompt is sent to Claude. First it clears the statusline's attention marker (`.cs/local/attention`) — any prompt, including slash commands, means the user is back — and, inside iTerm2 with shell integration installed, stops the dock bounce the Stop hook started (`CS_NO_ITERM2=1` disables), and, on macOS, removes the finished-turn notification the Stop hook posted, through the same poster — for the launched conversation only, since a tmux teammate takes prompts from the lead while the user is still away (`CS_NO_NOTIFY=1` disables). Then three responsibilities:
