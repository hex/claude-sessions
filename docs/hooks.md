...
Runs before each user prompt is sent to Claude. First it clears the statusline's attention marker (`.cs/local/attention`) — any prompt, including slash commands, means the user is back — and, inside iTerm2 with shell integration installed, stops the dock bounce the Stop hook started (`CS_NO_ITERM2=1` disables), and, on macOS with `terminal-notifier` on PATH, removes the finished-turn notification the Stop hook posted — for the launched conversation only, since a tmux teammate takes prompts from the lead while the user is still away (`CS_NO_NOTIFY=1` disables). Then three responsibilities:
